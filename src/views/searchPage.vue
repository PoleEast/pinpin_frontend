    ...
    <v-row>
      ...
      <v-col cols="12" md="9">
        ...
        <v-card elevation="2">
          ...
          <v-card-subtitle class="mb-2" v-if="locations.length">
          ...
          <v-card-actions class="d-block">
            <template v-if="!nextPageToken && lastTextSearchOption">
              <v-alert
                ...
                text="看來我們已經把這裡的寶藏都挖出來了！試試換個關鍵字，說不定會有意外驚喜"
                variant="plain"></v-alert>
            </template>
            <template v-else-if="nextPageToken && lastTextSearchOption">
              <div ref="nextpageLoaderTrigger">
                <v-alert
                  class="justify-center"
                  text="搜尋雷達全開！正在找更多好地方..."
                  variant="plain"></v-alert>
                <v-progress-linear
                  indeterminate
                  color="primary"
                  class="w-50 mx-auto mb-2"
                  opacity="0.8"></v-progress-linear>
              </div>
            </template>
            <template v-else>
              <v-alert
                class="justify-center"
                text="搜尋全世界的美食、景點、體驗，把想去的地方都收進行程裡！"
                variant="plain"></v-alert>
            </template>
          </v-card-actions>
        </v-card>
...
<script lang="ts" setup>
  ...
  watch(
    ...
    (query) => {
      if (
        !validatePriceLevel(query.priceLevel) ||
        !validatePrimaryType(query.primaryType) ||
        !validateRating(query.rating) ||
        !query.keyword
      ) {
