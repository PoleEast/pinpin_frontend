...
export const settingService = {
   ...
   */
  async GetSettingData(): Promise<
    AxiosResponse<ApiResponseDTO<SettingResponseDTO>>
  > {
  ...
  async GetUserProfile(): Promise<
    AxiosResponse<ApiResponseDTO<UserProfileResponseDTO>>
  > {
    const response: AxiosResponse<ApiResponseDTO<UserProfileResponseDTO>> =
      await axios.get("/userProfile/getUserProfile");

    if (
      response.data.data?.user?.createAt &&
      typeof response.data.data?.user?.createAt === "string"
    ) {
      response.data.data.user.createAt = new Date(
        response.data.data?.user?.createAt,
      );
    }
    return response;
  ...
  async GetDefaultAvatar(): Promise<
    AxiosResponse<ApiResponseDTO<AvatarResponseDTO[]>>
  > {
    const response: AxiosResponse<ApiResponseDTO<AvatarResponseDTO[]>> =
      await axios.get("/avatar/getDefaultAvatar");
  ...
  > {
    ...
    response.data.data = response.data.data?.map(
      (avatar: AvatarChangeHistoryResponseDTO) => {
