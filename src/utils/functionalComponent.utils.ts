...
const createTriangleIcon = (direction: "left" | "right" | "up" | "down"): FunctionalComponent => {
  return () =>
    h(FontAwesomeIcon, {
      icon: direction === "left" ? "caret-left" : direction === "right" ? "caret-right" : direction === "up" ? "caret-up" : "caret-down",
    });
};
...
const createNumberIcon = (num: number, size: FontAwesomeIconProps["size"] = "1x"): FunctionalComponent => {
  return () =>
    h(FontAwesomeIcon, {
      icon: num.toString(),
      size,
